(function () {
    ...
    BTree.prototype.insert = function (key, value) {
        // if (value === null) throw new Error("IllegalArgumentException : argument value to insert() is null");
        if(value === null || value === undefined){
            value = key;
        }

        let u = this.insert2(this.root, key, value, this.height);
        this.size++;
        if (u == null) return;

        // need to split root
        let t = new BTree.Node(2);
        t.children[0] = new BTree.Entry(this.root.children[0].key, this.root.children[0].value, this.root);
        t.children[1] = new BTree.Entry(u.children[0].key, u.children[0].value, u);
        this.root = t;
        this.height++;
    }
    BTree.prototype.insert2 = function (node, key, value, ht) {
        let i;
        let t = new BTree.Entry(key, value, null);

        if (ht == 0) {  // external node
            for (i = 0; i < node.size; i++) {
                if (node.children[i].compare(key) > 0) break;
            }
        } else {        // internal node
            for (i = 0; i < node.size; i++) {
                if (((i + 1) === node.size) || (node.children[i + 1].compare(key) > 0)) {
                    let u = this.insert2(node.children[i++].nextNode, key, value, ht - 1);   //node type
                    if (u === null) {
                        return null;
                    }
    ...
    BTree.prototype.get = function (key) {
        ...
        let result = this.search(this.root, key, this.height);
        if (result !== null) {
            return result.value;
        } else {
            return null;
    ...
    BTree.prototype.search = function (node, key, ht) {
        ...
        // internal node
            ...
            for (let i = 0; i < node.size; i++) {
                if (((i + 1) === node.size) || (children[i + 1].compare(key) > 0)) {
    ...
    BTree.Entry = function (key, value, nextNode) {
        this.key = key;
        this.value = value;
        this.nextNode = nextNode;
    }

    BTree.Entry.prototype.compare = function (key) {
        return this.key - key;
    }
