...
test("graph.removeVertex(vertex), also remove edges that contains 'vertex'",()=>{
    ...
    expect(graph.edgeSize).toBe(7);
});

test("graph.getDistance(vs, ve)",()=>{
    expect(graph.getDistance(v1,v1)).toBe(0);
    expect(graph.getDistance(v4,v4)).toBe(0);
    ...
    expect(graph.getDistance(v5,v1)).toBe(4);
    expect(graph.getDistance(v6,v1)).toBe(5);
});
...
test("graph.getPath(vs, ve)",()=>{
    ...
    expect(graph.getPath(v6,v1)).toEqual([v6,v5,v3,v1]);
    expect(graph.getPath(v5,v1)).toEqual([v5,v3,v1]);
});
